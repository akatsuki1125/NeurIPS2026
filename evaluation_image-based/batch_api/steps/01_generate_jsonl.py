...
log_file = Path("evaluation_image-based/batch_api/log/evaluation_step_generate_jsonl.log")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate Batch API input JSONL for evaluation."
    )
    parser.add_argument("--config", required=True, help="Path to experiment config (TOML).")
    parser.add_argument(
        "--config-name",
        default=None,
        help="Experiment name to run (all if omitted).",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Number of metadata rows to process from the top (all if omitted).",
    )
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Show only the first request as a preview (do not write file).",
    )
    parser.add_argument(
        "--amt-csv",
        type=str,
        default=None,
        help="AMT input CSV used to extract target keys (from img_edited_url).",
    )
    parser.add_argument(
        "--strict-csv-rows",
        action="store_true",
        help="Enforce strict row-level matching with CSV (error on missing rows).",
    )
    parser.add_argument(
        "--gemini-use-local",
        action="store_true",
        help="Send Gemini images as local inlineData instead of URL.",
    )
