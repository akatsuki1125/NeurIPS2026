...
```bash
python evaluation_image-level/batch_api/steps/01_generate_jsonl.py \
  --config evaluation_image-level/batch_api/configs/example.toml \
  --config-name example_experiment

python evaluation_image-level/batch_api/steps/02_run_batch.py \
  --config evaluation_image-level/batch_api/configs/example.toml \
  --config-name example_experiment

python evaluation_image-level/batch_api/steps/03_extract_scores.py \
  --config evaluation_image-level/batch_api/configs/example.toml \
  --config-name example_experiment
```
